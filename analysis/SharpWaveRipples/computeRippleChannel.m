...
function rippleChannels = computeRippleChannel(varargin)
...
% if exists subsession analysis

if ~isempty(foldername)
    filename = dir('*.channelinfo.SubSession.ripples.mat');
    if ~isempty(filename) && ~force
        if length(filename) >1
            ...
            if any(strcmpi(name,foldername))
                    disp(['Ripple channel already detected ! Loading file: ', filename(strcmpi(name,foldername)).name])
            ...
            if strcmpi(name,foldername)
                disp(['Ripple channel already detected ! Loading file: ', filename.name])
                load(filename.name)
                return
            end
        end
...
Anatomical_groups = sessionInfo.AnatGrps;
disp('Reading sessionInfo /  Anatomical Groups ')
disp('...                                  ')
...
% catch
%     if ~isempty(timestamps_subSession)
%         [ripples] = bz_FindRipples(basePath,RefrenceRippleChannel_test,'timestamps_subSession',timestamps_subSession,'foldername',foldername);
%     else
...
for shk=1:length(SHANKS)
    ...
    for CH=1:length(SHANKS{shk})
        ...
        if ~isempty(timestamps_subSession)
            eeg = single(lfp_aux.data(:,SHANKS{1,shk}(CH)));
        else
