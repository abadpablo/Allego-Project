function [transitions] = analysisTransitions_TMaze(transition, varargin)
%
% Goes one-by-one step defining triades and
% correct vs uncorrect trials for TMaze paradigm
% USAGE
%   [transitions] = analysisTransitions_TMaze(transition,varargin)
...
frames_leftReward = find(transition(1,:) == 1);
frames_rightReward = find(transition(1,:) == 2);
frames_decision = find(transition(1,:) == 3);
frames_starting = find(transition(1,:) == 4);
