...
% average_frame_x = nanmean(x);
% average_frame_y = nanmean(y);
...
if isfield(tracking,'apparatus') && isfield(tracking.apparatus,'boundingbox')
    linMazeCont = [tracking.apparatus.boundingbox.xmin tracking.apparatus.boundingbox.xmax];
else
    disp('No apparatus and boundingbox variables...Taking min(x) and max(x)')
    linMazeCont = [min(x) max(x)];
end


% We need to change this variable for the maze measures
...
digitalIn = bz_getDigitalIn('all','fs',30000);
...
if length(leftSensorTimes) == length(rightSensorTimes)
    armChoice.timestamps = [leftSensorTimes; rightSensorTimes];
    armChoice.visitedArm = [ones(size(leftSensorTimes)); zeros(size(rightSensorTimes))];
    [armChoice.timestamps, idx] = sort(armChoice.timestamps);
    armChoice.visitedArm = armChoice.visitedArm(idx);
elseif length(leftSensorTimes) > length(rightSensorTimes)
    armChoice.timestamps = [leftSensorTimes(1:end-1); rightSensorTimes];
    armChoice.visitedArm = [ones(size(leftSensorTimes,1),size(leftSensorTimes,2)-1); zeros(size(rightSensorTimes))];
    [armChoice.timestamps, idx] = sort(armChoice.timestamps);
    armChoice.visitedArm = armChoice.visitedArm(idx);
end
...
colormap parula
colorTraj = jet(size(armChoice.timestamps,2)*2);
prev = 0; 
% direction = ones(size(linCont)); % left to right is 1
direction = ones(size(linCont)); % righ to left is 1
winTrial = []; 
arm = ones(size(linCont));
for ii = 1:size(armChoice.timestamps,2)*2
...
saveas(h2,'linearizeTrajectory.png');
...
trials0 = [armChoice.timestamps(1:end); [armChoice.timestamps(2:end) t(end)]]; % trials defined as epochs between sensors crossing
% trials0 = [armChoice.timestamps [armChoice.timestamps(end); t(end)]]; % trials defined as epochs between sensors crossing
...
for ii = 1:length(trials0)
    posTrials = find(tracking.timestamps >= trials0(1,ii) & tracking.timestamps <= trials0(2,ii));
    trialMask(posTrials) = ii;
    trialMaskDirection(ii) = round(mean(arm(posTrials)));
end
