function [placeFieldStats] = bz_findPlaceFields1D(conditions,varargin)
...
for unit = 1:length(firingMaps.rateMaps)     
    for c = 1:length(conditions)
        % Default values
        mapStats{unit,1}{conditions(c)}.x = NaN;
        mapStats{unit,1}{conditions(c)}.field = [];
        mapStats{unit,1}{conditions(c)}.size = 0;
        mapStats{unit,1}{conditions(c)}.peak = 0;
        mapStats{unit,1}{conditions(c)}.mean = 0;
        mapStats{unit,1}{conditions(c)}.fieldX = [NaN NaN];
        mapStats{unit,1}{conditions(c)}.specificity = 0;
        mapStats{unit,1}{conditions(c)}.m = nan;
        mapStats{unit,1}{conditions(c)}.r = nan;
        mapStats{unit,1}{conditions(c)}.mode = nan;
        mapStats{unit,1}{conditions(c)}.k = nan;
        ...
        z = firingMaps.rateMaps{unit}{conditions(c)};
        x = 1:length( firingMaps.rateMaps{1}{conditions(c)});
...
if saveMat
   save([basepath,filesep,placeFieldStats.sessionName '.placeFields1D.cellinfo.mat'],'placeFieldStats'); 
end
...
for unit = 1:length(firingMaps.rateMaps)
    ...
    for c = 1:length(conditions)
        subplot(2,2,c)
        plot(firingMaps.rateMaps{unit}{conditions(c)},'k')
        if sum(firingMaps.rateMaps{unit}{conditions(c)})>0
            hold on
            for ii = 1:size(mapStats{unit}{conditions(c)}.field,2)
                plot(find(mapStats{unit}{conditions(c)}.field(:,ii)),firingMaps.rateMaps{unit}{conditions(c)}(mapStats{unit}{conditions(c)}.field(:,ii)==1),'linewidth',2)
                plot([1 1]*mapStats{unit}{conditions(c)}.x(ii),[0 firingMaps.rateMaps{unit}{conditions(c)}(mapStats{unit}{conditions(c)}.x(ii)==1)],'--k')
            end
        end
...
for c = 1:length(conditions)
    ...
    saveas(gcf,[basepath,filesep,'newPCs',filesep ,'map_' num2str(conditions(c)) '.png'],'png');
end
