function [firingMaps] = bz_firingMapAvg_v2_abad(positions,spikes,varargin)
...
addParameter(p,'nBins',40,@isnumeric);
...
%     positions{iCond}(v<speedThresh,:) = []; % Eliminated by me
end
...
%             'nBins',nBins,'maxGap',maxGap,'mode',mode,'maxDistance',maxDistance);
        map{unit}{c} = generateMaps_abad(positions{c},spikes.times{unit},'smooth',smooth,'minTime',minTime,...
            'nBins',nBins,'maxGap',maxGap,'mode',mode,'maxDistance',maxDistance);
    end
end
