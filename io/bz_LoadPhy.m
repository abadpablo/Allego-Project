...
if exist([basepath filesep sessionInfo.FileName '.spikes.cellinfo.mat'],'file') && forceReload == false
    ...
    channel_shanks = readNPY(fullfile(kilosort_path,'channel_shanks.npy'));
    try
        shanks = readNPY(fullfile(kilosort_path, 'shanks.npy')); % done
%         shanks = readNPY(fullfile(kilosort_path, 'channel_shanks.npy')); % created by Abad
...
%             spikes.shankID(jj) = shanks(cluster_id);
    ...
    wfWin = round((wfWin * fs)/2);
        for ii = 1 : size(spikes.times,2)
            ...
            for i=1:max(unique(channel_shanks))
                if any(ismember(sessionInfo.AnatGrps(i).Channels,spikes.maxWaveformCh(ii)))
                    spikes.shankID(ii) = i;
                end
            end
