function session = loadXDatMetadata(session,basepathIn)
...
if exist(fullfile(basepath,'info.dat'),'file')
...
elseif exist(fullfile(basepath,'info.xdat.json'),'file')
    if exist('read_infoxdatjson','file')
        Intan_rec_info = read_infoxdatjson(basepath);
        if isfield(session,'timeSeries') && iscell(session.timeSeries)
            warning('session.timeSeries not formatted correctly. Overwriting existing structure.')
            session.timeSeries = [];
        end
