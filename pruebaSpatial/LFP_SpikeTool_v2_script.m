...
function  [ci2 SpikeAmpD WidthD  MeanF meanAn InfInter SupInter mvl z p  NSpikes totalUse oo]=LFP_SpikeTool_v2_script(SK1,CUTO1,duration,CA1,Cell1,ExperCell,spiketimes,filspeed)
% % (C) Jorge R. Brotons-Mas (2015)
% % This script is and attemp to obtain the relationship between spike
% % ocurrance and theta phase. 
...
%     end
% % filename=[filenamCon1,'.egf',num2str(ch)]
% [CA1, Fs] = getegf('D:\ERB4\R27\0304-ERB4-R27\0304-ERB4-R27_OF1_Mrg_C.egf3');
...
%  for kl=2:2:(length(tstep))/Args.Fs-2
  for kl=2:1:(length(tstep))/Args.Fs-2 %%% Hamming
...
% th=filtfilt(BC,[1],y) ;%theta band filtered
 ...
 N=round(length(y)/3);
     y = y.*hamming(numel( y ))' ;
    bRP1  = firls(N, [0 Fstop1 Fpass1 Fpass2 Fstop2 Fs/2]/(Fs/2), [0 0 1 1 0 0], [Wstop1 Wpass Wstop2]);   
    th = filtfilt(bRP1,1,y);
...
if (RatTD >10) &&  (max(thetaEnv_ep)< Threstheta)  % Only Epochs of strong theta are included for analysis eliminating high amplitude artifacts
...
% VectimeSpike=find(CellTS1>=Eptime(1) & CellTS1<=Eptime(end)); No hamming

VectimeSpike=find(CellTS1>=Eptime(1)+(Eptime(end)-Eptime(1))/4 & CellTS1<=Eptime(1)+3*(Eptime(end)-Eptime(1))/4);% Hamming
...
figure;bar(oo/NSpikes);title([ 'Cell',num2str(ci2),'Phase Histogram'])
...
%  OutDeg(ci2,1:end )=
% ci2 SpikeAmpD WidthD  MeanF meanAn InfInter SupInter mvl z p  NSpikes totalUse ];
